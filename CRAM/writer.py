...
class FileWriter():
    ...
    def write(self, sparse_matrix):
        ...
        with open(self.filepath, 'wb') as f:
            # Headers
            f.write(b'CRAM')
            f.write(struct.pack('i', int(self.core.version)))
            f.write(struct.pack('i', int(rows)))
            f.write(struct.pack('i', int(cols)))
            f.write(struct.pack('q', int(nnz)))
            f.write(struct.pack('q', int(self.core.header_size)))
            f.write(struct.pack('q', int(data_offset)))
            ...
            for row_idx, start in enumerate(sparse_matrix.indptr[:-1]):
                ...
                f.write(struct.pack('i', int(row_idx)))
                f.write(struct.pack('q', int(offset)))
                f.write(struct.pack('i', int(nnz_in_row)))
